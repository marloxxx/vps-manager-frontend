...
- **🔒 SSL Management**: Handle certificate management and SSL configuration.
- **📈 System Monitoring**: View real-time system stats, resource usage, and health checks.
- **📝 Log Viewer**: Browse and download Nginx access and error logs.
- **💾 Backup & Restore**: Manage configuration backups and restores.
...
- **VPS Manager Backend**: Running and accessible (e.g., at `http://your-server-ip:8000`).
...
git clone https://github.com/marloxxx/vps-manager-frontend.git
...
```env
# Backend API URL - Update to point to your backend server
...
3. **Create Configuration**: Click "New Configuration" to set up a reverse proxy.
4. **Monitor System**: Check system health and view logs.
5. **Manage SSL**: Configure SSL certificatesviolent for secure connections.
...
3. Select a template, customize as needed, and deploy.

### Managing Load Balancers
1. Navigate to the **Load Balancer** tab.
...
### API Integration
The frontend communicates with the VPS Manager Backend via RESTful APIs:
...
4. Configure a custom domain in Vercel settings.
...
2. **Add Routes**: Create pages in `app/`.
